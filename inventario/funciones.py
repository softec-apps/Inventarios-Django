...
def exportar_productos_csv(request):
    ...
    writer.writerow(
        ["ID", "Descripcion", "Precio", "Disponible", "Medida", "Categoria"]
    )

    productos = Producto.objects.all().values_list(
        "id", "descripcion", "precio", "disponible", "medida", "categoria__nombre"
    )
    ...
    for entry in kardex_entries:
        writer.writerow(
            [
                ...
                entry.valor_total,
                entry.saldo_cantidad,
                entry.saldo_valor_total,
                entry.detalle,
            ]
        )
...
def exportar_kardex_producto_csv(request, producto_id):
    # Filtrar entradas del Kardex por el producto solicitado
    producto = Producto.objects.get(id=producto_id)
    kardex_entries = Kardex.objects.filter(producto=producto).select_related("producto")

    response = HttpResponse(content_type="text/csv")
    response["Content-Disposition"] = f'attachment; filename="kardex_{producto.descripcion}.csv"'
...
def exportar_kardex_producto_excel(request, producto_id):
    # Filtrar entradas del Kardex por el producto solicitado
    producto = Producto.objects.get(id=producto_id)
    kardex_entries = Kardex.objects.filter(producto=producto).select_related("producto")
    ...
    response["Content-Disposition"] = f'attachment; filename=kardex_{producto.descripcion}.xlsx'
    wb.save(response)
    return response
...
def exportar_proveedores_csv(request):
    ...
    for proveedor in proveedores:
        writer.writerow(
            [
                proveedor.id,
                proveedor.get_tipoCedula_display(),
                proveedor.cedula,
                proveedor.nombre,
                proveedor.apellido,
                proveedor.ciudad,
...
def exportar_pedidos_csv(request):
    ...
    writer = csv.writer(response)
    writer.writerow(
        ["ID", "Proveedor", "Fecha", "Sub Monto", "Monto General", "Recibido"]
    )
    ...
    for pedido in pedidos:
        writer.writerow(
            [
                pedido.id,
                f"{pedido.proveedor.nombre} {pedido.proveedor.apellido}",
                pedido.fecha,
                pedido.sub_monto,
                pedido.monto_general,
                "Si" if pedido.presente else "No",
            ]
        )
...
def exportar_clientes_csv(request):
    ...
    for cliente in clientes:
        writer.writerow(
            [
                cliente.id,
                cliente.get_tipoCedula_display(),
                cliente.cedula,
                cliente.nombre,
                cliente.apellido,
                cliente.ciudad,
...
def exportar_descuentos_csv(request):
    ...
    for descuento in descuentos:
        writer.writerow([descuento.id, descuento.nombre, descuento.valor])
...
def exportar_descuentos_excel(request):
    ...
    for descuento in descuentos:
        ws.append([descuento.id, descuento.nombre, descuento.valor])
...
def exportar_usuarios_csv(request):
    ...
    writer = csv.writer(response)
    writer.writerow(
        ["ID", "Username", "Email", "Nombre", "Apellido", "Nivel", "Es Superusuario"]
    )
